...
module.exports = Teatime = function(initURL, options) {
  ...
  this.resultDir = path.resolve(__dirname, 'file', this.startUrlParsed.hostname);
  this.resultPath = path.join(this.resultDir, 'theData.json');

  fs.ensureDirSync(this.resultDir);
  fs.writeFileSync(this.resultPath, '{\n', 'utf-8');
...
Teatime.prototype.crawl = function() {
  ...
  if(this.options.crawl == true) {
    if(pending.length > 0) {
      ...
      .then(function() {
        setTimeout(function () {
          fs.appendFileSync(that.resultPath, '}', 'utf8');
          that.end();
        }, 5000);
      });
    }
...
Teatime.prototype.end = function () {
    var loaded = fs.readFileSync(this.resultPath, 'utf8');
    
    loaded = loaded.substr(0, loaded.length - 3) + '}';
    this.deferred.resolve(JSON.parse(loaded));
}; 